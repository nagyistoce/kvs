...
//    m_volume->create( m_nsteps, nvertices, ncells );
    ...
    const kvs::Real32* src_coord   = ref_volume[index]->coords().pointer();
    const kvs::UInt32* src_connect = ref_volume[index]->connections().pointer();
    const T* src_value = static_cast<const T*>( ref_volume[index]->values().pointer() );
...
{
#if defined( KVS_GLEW_STOCHASTIC_MULTIPLE_TETRAHEDRA_ENGINE__EMBEDDED_SHADER )
    const std::string vert_code = index == 0 ?
        kvs::glew::glsl::StochasticRenderingEngine::Vertex::multiple_tetrahedra1 :
        kvs::glew::glsl::StochasticRenderingEngine::Vertex::multiple_tetrahedra2;
    const std::string geom_code = index == 0 ?
        kvs::glew::glsl::StochasticRenderingEngine::Geometry::multiple_tetrahedra1 :
        kvs::glew::glsl::StochasticRenderingEngine::Geometry::multiple_tetrahedra2;
    const std::string frag_code = index == 0 ?
        kvs::glew::glsl::StochasticRenderingEngine::Fragment::multiple_tetrahedra1 :
        kvs::glew::glsl::StochasticRenderingEngine::Fragment::multiple_tetrahedra2;
#else
    const std::string vert_code = index == 0 ?
        "StochasticRenderingEngine/multiple_tetrahedra1.vert" :
        "StochasticRenderingEngine/multiple_tetrahedra2.vert";
    const std::string geom_code = index == 0 ?
        "StochasticRenderingEngine/multiple_tetrahedra1.geom" :
        "StochasticRenderingEngine/multiple_tetrahedra2.geom";
    const std::string frag_code = index == 0 ?
        "StochasticRenderingEngine/multiple_tetrahedra1.frag" :
