...
{
    const kvs::Vector3f& S = m_scaling;
    const kvs::Matrix33f& R = m_rotation;
...
kvs::Matrix44f Xform::toMat4() const
{
    const kvs::Vector3f& S = m_scaling;
    const kvs::Matrix33f& R = m_rotation;
    const kvs::Vector3f& T = m_translation;
    return kvs::Matrix44f( S[0] * R[0][0], S[1] * R[0][1], S[2] * R[0][2], T[0],
