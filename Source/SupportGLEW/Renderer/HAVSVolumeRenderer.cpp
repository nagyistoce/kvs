...
#include "HAVSVolumeRenderer/Shader.h"
#endif
...
void PartialSort(
    int byte,
    int length,
    kvs::glew::HAVSVolumeRenderer::SortedFace* src,
    kvs::glew::HAVSVolumeRenderer::SortedFace* dst,
    int* count )
{
    register unsigned int i;
    kvs::glew::HAVSVolumeRenderer::SortedFace* k;
...
{
    bool operator() (
        const kvs::glew::HAVSVolumeRenderer::Face& f1,
        const kvs::glew::HAVSVolumeRenderer::Face& f2 ) const
    {
...
void HAVSVolumeRenderer::initialize_shader( void )
{
    ...
    const std::string vert_code_kbuffer = kvs::glew::glsl::HAVSVolumeRenderer::Vertex::kbuffer;
#else
    const std::string vert_code_begin = "HAVSVolumeRenderer/begin.vert";
    const std::string vert_code_kbuffer = "HAVSVolumeRenderer/kbuffer.vert";
#endif

    if ( this->kBufferSize() == 2 )
    {
#if defined( KVS_GLEW_HAVS_VOLUME_RENDERER__EMBEDDED_SHADER )
        const std::string frag_code_begin = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k2_begin;
        const std::string frag_code_end = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k2_end;
        const std::string frag_code_kbuffer = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k2;
#else
        const std::string frag_code_begin = "HAVSVolumeRenderer/k2_begin.frag";
        const std::string frag_code_end = "HAVSVolumeRenderer/k2_end.frag";
        const std::string frag_code_kbuffer = "HAVSVolumeRenderer/k2.frag";
#endif
        kvs::glew::ShaderSource vert_begin( vert_code_begin );
        kvs::glew::ShaderSource vert_kbuffer( vert_code_kbuffer );
    ...
    {
#if defined( KVS_GLEW_HAVS_VOLUME_RENDERER__EMBEDDED_SHADER )
        const std::string frag_code_begin = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k6_begin;
        const std::string frag_code_end = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k6_end;
        const std::string frag_code_kbuffer = kvs::glew::glsl::HAVSVolumeRenderer::Fragment::k6;
#else
        const std::string frag_code_begin = "k6_begin.frag";
        const std::string frag_code_end = "k6_end.frag";
        const std::string frag_code_kbuffer = "k6.frag";
#endif
        kvs::glew::ShaderSource vert_begin( vert_code_begin );
        kvs::glew::ShaderSource vert_kbuffer( vert_code_kbuffer );
        kvs::glew::ShaderSource frag_begin( frag_code_begin );
...
void HAVSVolumeRenderer::sort_geometry( kvs::Camera* camera )
{
    ...
    if ( this->isEnabledVBO() )
    {
        m_pindices = static_cast<GLuint*>( m_vertex_indices.map( kvs::glew::IndexBufferObject::WriteOnly ) );
    }
